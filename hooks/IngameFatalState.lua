function IngameFatalState:update(t, dt)
	...
	if self._anim_time then
		...
		if self._anim_stage == 2 then
			self._cam_unit:camera():modify_fov(dt * 30)
		...
		elseif self._anim_stage == 5 then
			self._cam_unit:camera():modify_fov(-dt * 40)
		end
		if self._anim_time <= 0 then
			...
			elseif self._anim_stage == 2 then
				...
				managers.hud._hud_access_camera:wz_connection_text("hud_access_camera_reconnecting")
			elseif self._anim_stage == 3 then
				managers.hud._hud_access_camera:wz_connection_text("hud_access_camera_reconnected")
...
function IngameFatalState:enable_camera()
	...
	self._cam_unit:camera():modify_fov(-30)
